...
void Shoot::Update()
{
	...
	if (laserActive <= 0)
	{
			...
			//if (Input::GetKeyDown(SDLK_SPACE))
		{
			...
			laser = Instantiate(game->Prefab("Laser"), gameObject->transform->GetAbsolutePosition() - 420 * gameObject->transform->Up() - 0 * gameObject->transform->Right(), gameObject->transform->GetAbsoluteAngle() + 180);
	...
	{
		laser->transform->SetAbsolutePosition(gameObject->transform->GetAbsolutePosition() - 420 * gameObject->transform->Up() - 0 * gameObject->transform->Right()); // Update laser
